...
const places = [[{ city: 'Seoul', time: '15-minute', picture: seoul },
...
const Place = ({ city, time, picture }: Props) => (
...
const ExploreNearby = () => {
  const keyExtractor = useCallback((item: Props[]) => item[0].city + item[1].city, []);
  const renderItem = useCallback(({ item }: {item: Props[]}) => (
    <View>
      <Place {...item[0]} />
      <Place {...item[1]} />
    </View>
